...
const oldPrio = `const PRIORITY_META = {
...
const newPrio = `const PRIORITY_META = {
...
text = text.replace(oldPrio, newPrio);

const cssStart = text.indexOf('const CSS = `');
const cssEnd = text.indexOf('`;', cssStart) + 2;
...
text = text.replace(/var\(--mono\)/g, 'var(--font-body)');
text = text.replace(/var\(--serif\)/g, 'var(--font-head)');
