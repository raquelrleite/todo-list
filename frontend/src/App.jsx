...
const OAUTH2_ERRORS = {
  email_not_available: "Your account's email is not public. Please make it public in your provider settings and try again.",
  oauth2: "Social login failed. Please try again.",
};
...
function useApi() {
  const request = useCallback(async (method, path, body = null, retry = true) => {
    ...
    if (res.status === 401 && retry && !path.includes("/login") && !path.includes("/refresh")) {
...
function RichTextEditor({ value, onChange }) {
  ...
  const ToolBtn = ({ action, active, title, children }) => (
    <button type="button" onClick={action} title={title} style={{ background: active ? "rgba(139,110,232,0.15)" : "none", border: "none", borderRadius: 5, padding: "4px 7px", cursor: "pointer", color: active ? "var(--accent)" : "var(--muted)", display: "flex", alignItems: "center", transition: "all 0.12s" }}>
      {children}
    </button>
  );
  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: "var(--r)", overflow: "hidden", background: "var(--surface)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 1, padding: "5px 8px", borderBottom: "1px solid var(--border)", background: "var(--card)" }}>
        <ToolBtn action={() => editor.chain().focus().toggleBold().run()}        active={editor.isActive("bold")}        title="Bold">        <Icon d={I.bold}   size={13} /></ToolBtn>
        <ToolBtn action={() => editor.chain().focus().toggleItalic().run()}      active={editor.isActive("italic")}      title="Italic">      <Icon d={I.italic} size={13} /></ToolBtn>
        <Sep />
        <ToolBtn action={() => editor.chain().focus().toggleBulletList().run()}  active={editor.isActive("bulletList")}  title="Bullet list">  <Icon d={I.listUl} size={13} /></ToolBtn>
        <ToolBtn action={() => editor.chain().focus().toggleOrderedList().run()} active={editor.isActive("orderedList")} title="Numbered list"> <Icon d={I.listOl} size={13} /></ToolBtn>
        <Sep />
        <ToolBtn action={() => editor.chain().focus().undo().run()} title="Undo"><Icon d={I.undo} size={13} /></ToolBtn>
        <ToolBtn action={() => editor.chain().focus().redo().run()} title="Redo"><Icon d={I.redo} size={13} /></ToolBtn>
      </div>
      <EditorContent editor={editor} style={{ padding: "10px 14px", minHeight: 80, color: "var(--text)", fontSize: 13, lineHeight: 1.6 }} />
    </div>
  );
}
...
function DateTimePicker({ value, onChange }) {
  ...
  useEffect(() => {
    ...
    if (!value) { setSel(null); setStep("cal"); }
    else if (parsed) {
      setSel({ y: parsed.y, m: parsed.m, day: parsed.day });
      setHour(parsed.h); setMinute(parsed.min);
  ...
  const pickDay   = (day) => { const s = { y: cursor.y, m: cursor.m, day }; setSel(s); emit(s, hour, minute); setStep("time"); };
  const changeH   = (d) => { const h = (hour   + d + 24) % 24; setHour(h);   if (sel) emit(sel, h, minute); };
  const changeM   = (d) => { const m = (minute + d + 60) % 60; setMinute(m); if (sel) emit(sel, hour, m); };
  ...
  const cells    = Array.from({ length: Math.ceil((startCol + total) / 7) * 7 }, (_, i) => {
...
function Alert({ color, bg, border, children }) {
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 10, padding: "12px 14px", marginBottom: 12 }}>
      <p style={{ color, fontSize: 12 }}>{children}</p>
    </div>
  );
}

function Auth({ onLogin, onForgot, oauthError }) {
  ...
  const go = async () => {
    ...
    if (tab === "up" && f.pass !== f.passConfirm) return setErr("Passwords do not match");
    setBusy(true);
    try {
      if (tab === "up") {
        await request("POST", "/v1/users/register", { name: f.name, email: f.email, password: f.pass, passwordConfirmation: f.passConfirm });
        setRegistered(true);
        setTab("in");
        return;
      }
      ...
      e.message?.toLowerCase().includes("verify") ? setNotVerified(true) : setErr(e.message);
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="auth-wrap">
      <div className="auth-box">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
          <a href={`${API_URL}/oauth2/authorization/google`} style={socialStyle}
            onMouseEnter={e => e.currentTarget.style.borderColor = "var(--muted)"}
            onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}>
            <GoogleIcon /> Continue with Google
          </a>
          <a href={`${API_URL}/oauth2/authorization/github`} style={socialStyle}
            onMouseEnter={e => e.currentTarget.style.borderColor = "var(--muted)"}
            onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          <span style={{ color: "var(--muted2)", fontSize: 11 }}>or</span>
          <div style={{ flex: 1, height: 1, background: "var(--border)" }} />
        </div>
        ...
        <div className="field">
          ...
          <input data-f type="password" value={f.pass} onChange={upd("pass")} placeholder="••••••••" onKeyDown={e => e.key === "Enter" && go()} />
        </div>
        {tab === "up" && (
          <div className="field">
        ...
        {registered  && <Alert color="var(--green)" bg="rgba(45,191,138,0.08)"  border="rgba(45,191,138,0.3)">  ✓ Account created! Check your email to confirm before signing in.</Alert>}
        {err         && <Alert color="var(--red)"   bg="rgba(224,85,153,0.07)"  border="rgba(224,85,153,0.25)">{OAUTH2_ERRORS[err] || err}</Alert>}
        {resendDone  && <Alert color="var(--green)" bg="rgba(45,191,138,0.08)"  border="rgba(45,191,138,0.3)">  ✓ Confirmation email sent. Check your inbox.</Alert>}
        {notVerified && (
          <div style={{ background: "rgba(139,110,232,0.08)", border: "1px solid rgba(139,110,232,0.25)", borderRadius: 10, padding: "12px 14px", marginBottom: 12 }}>
            <p style={{ color: "var(--text)", fontSize: 12, marginBottom: 8 }}>Please verify your email before signing in.</p>
        ...
        <button className="btn btn-p btn-w" onClick={go} disabled={busy}>
...
function OAuth2Callback({ onLogin, onError }) {
  useEffect(() => {
  ...
  return (
    <div className="auth-wrap">
      <div className="auth-box" style={{ textAlign: "center" }}>
        <div className="logo">taskflow.</div>
        <p style={{ color: "var(--muted)", fontSize: 13, marginTop: 24 }}>Signing you in…</p>
      </div>
    </div>
  );
}
...
function AccountConfirmation({ token, onBack }) {
  ...
  useEffect(() => {
    if (!token) return;
    request("GET", `/v1/users/account-confirmation?token=${token}`)
  ...
  return (
    <div className="auth-wrap">
      <div className="auth-box" style={{ textAlign: "center" }}>
        <div className="logo">taskflow.</div>
        {status === "loading" && <p style={{ color: "var(--muted)", fontSize: 13, marginTop: 20 }}>Confirming your account…</p>}
        {status === "success" && (
          <>
...
function ForgotPassword({ onBack }) {
  ...
  const go = async () => {
    ...
    setErr(""); setBusy(true);
    try { await request("POST", "/v1/users/forgot-password", { email }); setDone(true); }
    catch (e) { setErr(e.message); }
    finally { setBusy(false); }
  };
      ...
      <div className="auth-box">
        ...
        {done ? (
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ color: "var(--muted)", fontSize: 12, marginBottom: 24 }}>If an account with that email exists, a reset link has been sent.</p>
            <button className="btn btn-g btn-w" onClick={onBack}>Back to Sign In</button>
          </div>
        ) : (
          <>
            <p style={{ color: "var(--muted)", fontSize: 12, marginBottom: 20 }}>Enter your email and we'll send you a link to reset your password.</p>
            <div className="field">
              <label className="label">Email</label>
              <input data-f type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" autoFocus onKeyDown={e => e.key === "Enter" && go()} />
            </div>
...
function ResetPassword({ token, onBack }) {
  ...
  const go = async () => {
    ...
    setErr(""); setBusy(true);
    try { await request("POST", "/v1/users/reset-password", { token, password: f.pass, passwordConfirmation: f.passConfirm }); setDone(true); }
    catch (e) { setErr(e.message); }
    finally { setBusy(false); }
  };
      ...
      <div className="auth-box">
        ...
        {done ? (
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ color: "var(--muted)", fontSize: 12, marginBottom: 24 }}>Your password has been changed successfully.</p>
            <button className="btn btn-p btn-w" onClick={onBack}>Sign In</button>
          </div>
        ) : (
          <>
            <div className="field"><label className="label">New Password</label><input data-f type="password" value={f.pass} onChange={upd("pass")} placeholder="••••••••" autoFocus /></div>
            <div className="field"><label className="label">Confirm New Password</label><input data-f type="password" value={f.passConfirm} onChange={upd("passConfirm")} placeholder="••••••••" onKeyDown={e => e.key === "Enter" && go()} /></div>
            {err && <div className="err" style={{ marginBottom: 12 }}>{err}</div>}
...
function CatModal({ cat, onClose, onSave }) {
  ...
  const save = async () => {
    ...
    try {
      cat
        ? await request("PATCH", `/v1/categories/${cat.id}`, { title })
...
function TaskModal({ task, cats, onClose, onSave }) {
  ...
  const isEmpty = (html) => !html || html.trim() === "" || html.trim() === "<p></p>";
  ...
  return (
    <div className="ov" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="mh">
          <div className="mt">{isEdit ? "Edit Task" : "New Task"}</div>
          <button className="ib" onClick={onClose}><Icon d={I.x} /></button>
        </div>
        <div className="mb">
          <div className="field" style={{ marginBottom: 0 }}>
          ...
          <div>
            ...
            <div className="pgrid">
              {PRIORITIES.map(p => {
                ...
                return (
                  <div key={p} className="popt" onClick={() => toggleP(p)}
        ...
        <div className="mf">
          <button className="btn btn-g" onClick={onClose}>Cancel</button>
          <button className="btn btn-p" onClick={save}>{isEdit ? "Save Changes" : "Create Task"}</button>
        </div>
      </div>
    </div>
  );
...
function detectScreen() {
  ...
  if (path.includes("oauth2/callback"))               return { name: "oauth2callback" };
  ...
  if (oauthError) return { name: "auth", oauthError };

  return { name: "auth" };
}

export default function App() {
  const [authed,   setAuthed]   = useState(!!localStorage.getItem("name"));
  const [screen,   setScreen]   = useState(detectScreen);
  ...
  const logout = () => { clearAuthStorage(); setAuthed(false); setScreen({ name: "auth" }); };
  ...
  const shown   = tasks.filter(t => !search || [t.title, t.description].some(v => v?.toLowerCase().includes(search.toLowerCase())));
  const catName = actCat ? (cats.find(c => c.id === actCat)?.title || "Category") : null;

  if (screen.name === "confirm") return <><style>{CSS}</style><AccountConfirmation token={screen.token} onBack={goBack} /></>;
  ...
  if (screen.name === "oauth2callback") {
    return (
      <>
        <style>{CSS}</style>
        <OAuth2Callback
          onLogin={() => { setAuthed(true); setScreen({ name: "auth" }); }}
          onError={(err) => setScreen({ name: "auth", oauthError: err })}
        />
      </>
    );
  }
  ...
  if (!authed) {
    return <><style>{CSS}</style><Auth onLogin={() => setAuthed(true)} onForgot={() => setScreen({ name: "forgot" })} oauthError={screen.oauthError} /></>;
  }
